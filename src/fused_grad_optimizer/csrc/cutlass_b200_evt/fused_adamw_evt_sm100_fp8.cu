 ...
 * untouched.
 *
 * FP8 DESIGN — fp8_plan.md Change G:
 *   GEMM operand types swapped to e4m3 (1 byte each); accumulator stays fp32;
 *   W/m/v stay bf16. Per-tensor inverse scale (1/(s_GO*s_INP)) is FOLDED
 *   into the AdamW coefficients on host — no extra EVT compute node:
 *     g_real      = g_fp8 * scale_inv          (scale_inv = 1/(s_GO*s_INP))
 *     m_new       = ((1-β1)*scale_inv) * g_fp8 + β1*m_prev
 *     v_new       = ((1-β2)*scale_inv^2) * g_fp8^2 + β2*v_prev
 *   So host precomputes one_m_b1_scaled, one_m_b2_scaled and the existing
 *   m/v/w functors are reused unchanged. BF16 baseline kernel logic stays
 *   intact; only operand types and coefficient pre-scale change.
 *
 * Static per-tensor scaling: caller passes scale_inv as a single fp32 scalar.
 * (Delayed/dynamic scaling per fp8_plan.md is a follow-up — not needed for
 * a perf measurement against the bf16 baseline.)
 *
...
// ══════════════════════════════════════════════════════════════════════════
using ElementA           = cutlass::float_e4m3_t;          // FP8: GO_fp8^T
using LayoutA            = cutlass::layout::ColumnMajor;   // GO^T
using ElementB           = cutlass::float_e4m3_t;          // FP8: INP_fp8
...
struct BlackwellEVTAdamW {
  ...
  // Same fix as the bf16 sibling file: with AuxStages>=1 the M and V aux
  // nodes (both Sm90AuxLoad<Stages, EpilogueTile, bf16, ...> — identical
  // template type) collapse to a single SMEM region in the CUTLASS allocator,
  // so V's TMA load stomps M's. Step 1 was bit-exact only because m_load=0,
  // v_load=0 (corruption multiplied by 0); step 2+ produced wrong updates.
  // The Stages=0 specialisations have empty SharedStorage{} so each aux
  // reads/writes GMEM↔register directly with no cross-aux aliasing.
  //
  ...
  // ─── Launch (FP8) ───────────────────────────────────────────────────
  // Caller passes go/inp as ALREADY-QUANTIZED e4m3 tensors (uint8 storage)
  // plus a per-tensor scale_inv = 1/(scale_GO * scale_INP). Coefficients
  // are pre-scaled so the existing m/v functors work unchanged.
  static void launch(
      ...
      float scale_inv)
  {
    TORCH_CHECK(go.is_cuda()     && go.scalar_type()     == torch::kFloat8_e4m3fn && go.is_contiguous());
    TORCH_CHECK(inp.is_cuda()    && inp.scalar_type()    == torch::kFloat8_e4m3fn && inp.is_contiguous());
    ...
    // Precompute AdamW scalars on the host. Fold scale_inv into the m/v
    // coefficients so the kernel functors receive the right effective gradients.
    ...
    // Fold per-tensor scale_inv into AdamW state coefficients:
    //   m_new = ((1-β1)*scale_inv) * g_fp8 + β1*m_prev
    //   v_new = ((1-β2)*scale_inv^2) * g_fp8^2 + β2*v_prev
    const float one_m_b1       = (1.0f - beta1) * scale_inv;
    const float one_m_b2       = (1.0f - beta2) * scale_inv * scale_inv;
...
// ══════════════════════════════════════════════════════════════════════════
#define FP8_EVT_ADAMW_LAUNCHER(NAME, KIND)                                    \
  void blackwell_evt_adamw_fp8_##NAME(                                        \
      torch::Tensor go, torch::Tensor inp,                                    \
      torch::Tensor weight, torch::Tensor m, torch::Tensor v,                 \
      float lr, float beta1, float beta2, float eps, float weight_decay,      \
      float bc1, float bc2, float scale_inv)                                  \
  {                                                                           \
...
FP8_EVT_ADAMW_LAUNCHER(256x128_2sm,           256x128_2sm)
FP8_EVT_ADAMW_LAUNCHER(256x256_2sm,           256x256_2sm)
FP8_EVT_ADAMW_LAUNCHER(256x128_2sm_fast,      256x128_2sm_fast)
FP8_EVT_ADAMW_LAUNCHER(256x256_2sm_fast,      256x256_2sm_fast)


PYBIND11_MODULE(TORCH_EXTENSION_NAME, mod) {
  mod.def("blackwell_evt_adamw_fp8_256x128_2sm",
          &blackwell_evt_adamw_fp8_256x128_2sm,
          "FP8 (e4m3) Sm100 EVT-fused grad_W + AdamW (256x128x64, 2SM)");
  mod.def("blackwell_evt_adamw_fp8_256x256_2sm",
          &blackwell_evt_adamw_fp8_256x256_2sm,
          "FP8 (e4m3) Sm100 EVT-fused grad_W + AdamW (256x256x64, 2SM)");
  mod.def("blackwell_evt_adamw_fp8_256x128_2sm_fast",
          &blackwell_evt_adamw_fp8_256x128_2sm_fast,
          "FP8 (e4m3) + Change C (rsqrt+fma): atom 256x128, 2SM");
  mod.def("blackwell_evt_adamw_fp8_256x256_2sm_fast",
          &blackwell_evt_adamw_fp8_256x256_2sm_fast,
          "FP8 (e4m3) + Change C (rsqrt+fma): atom 256x256, 2SM");
}
